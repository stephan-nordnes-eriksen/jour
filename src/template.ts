...
export function journalTemplate(config: JournalConfig, title = "") {
	const currentDateString = config.currentTime.toLocaleDateString(config.locale)
	const journalTemplateData: TemplateData = {
		entryTitle: 'Journal',
		date: config.currentTime,
	...
	return Template.processTemplateName(config.template, journalTemplateData)
}
